    ...
    Navtab.prototype.openTab = function() {
        ...
        if (iOpenIndex >= 0) {
            ...
            if(!options.isIFrame) {
                ...
                var iframe = $('<iframe id="' + options.id + '" name="' + options.id + '" src="' + options.url + '" frameborder="0" scrolling="auto" />')
                    .css({ width: '100%', height: $panel.parent().height(), border: 0 }).appendTo($panel);
                //add a listener to the load event
                /*$('#' + options.id).addEvent('load', function() {

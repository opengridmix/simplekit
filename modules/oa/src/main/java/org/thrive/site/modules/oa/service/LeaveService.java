 ...
 */
package org.thrive.site.modules.oa.service;
...
import org.thrive.site.core.util.Collections3;
import org.thrive.site.core.util.StringUtil;
import org.thrive.site.modules.act.util.ActUtil;
import org.thrive.site.modules.oa.dao.LeaveDao;
...
public class LeaveService extends BaseService {
	...
	public void save(Leave leave, Map<String, Object> variables) {
		...
		ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(ActUtil.PD_LEAVE[0], businessKey, variables);
		...
		logger.debug("start process of {key={}, bkey={}, pid={}, variables={}}", new Object[] { 
				ActUtil.PD_LEAVE[0], businessKey, processInstance.getId(), variables });
	...
	public List<Leave> findTodoTasks(String userId) {
		...
		// 根据当前人的ID查询
		List<Task> todoList = taskService.createTaskQuery().processDefinitionKey(ActUtil.PD_LEAVE[0]).taskAssignee(userId).active().orderByTaskPriority().desc().orderByTaskCreateTime().desc().list();
		// 根据当前人未签收的任务
		List<Task> unsignedTasks = taskService.createTaskQuery().processDefinitionKey(ActUtil.PD_LEAVE[0]).taskCandidateUser(userId).active().orderByTaskPriority().desc().orderByTaskCreateTime().desc().list();
		// 合并

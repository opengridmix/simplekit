 ...
 */
package org.thrive.site.modules.act.service;
...
import org.thrive.site.modules.act.dao.ActDao;
import org.thrive.site.modules.act.util.ProcessDefCache;
...
public class ActTaskService extends BaseService {
	...
	public List<Act> todoList(Act act){
		String userId = UserUtil.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
					if (il.size() > 0){
						if (StringUtil.isNotBlank(il.get(0).getStartUserId())){
							User user = UserUtil.getByLoginName(il.get(0).getStartUserId());
							if (user != null){
				...
				if (StringUtil.isNotEmpty(histIns.getAssignee())){
					User user = UserUtil.getByLoginName(histIns.getAssignee());
					if (user != null){
						e.setAssignee(histIns.getAssignee());
						e.setAssigneeName(user.getName());
					}
	...
	public void completeFirstTask(String procInsId, String comment, String title, Map<String, Object> vars){
		String userId = UserUtil.getUser().getLoginName();
...
//	}
	...
	private Map<String, Object> packageSingleActivitiInfo(ActivityImpl activity, ProcessInstance processInstance,
		...
		vars.put("任务类型", ActUtil.parseToZhType(properties.get("type").toString()));

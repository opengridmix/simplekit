...
import org.thrive.sites.core.servlet.ValidateCodeServlet;
import org.thrive.sites.core.util.StringUtil;
import org.thrive.sites.core.controller.BaseController;
import org.thrive.sites.module.cms.entity.*;
import org.thrive.sites.module.cms.service.*;
...
public class FrontController extends BaseController{
	...
	public String index(Model model) {
		...
		return "module/cms/front/themes/"+site.getTheme()+"/frontIndex";
	}
	...
	public String index(@PathVariable String siteId, Model model) {
		...
		if (StringUtil.isNotBlank(site.getCustomIndexView())){
			model.addAttribute("site", site);
			model.addAttribute("isIndex", true);
			return "module/cms/front/themes/"+site.getTheme()+"/frontIndex"+site.getCustomIndexView();
		}
		// 否则显示子站第一个栏目
		List<Category> mainNavList = CmsUtil.getMainNavList(siteId);
		if (mainNavList.size() > 0){
			String firstCategoryId = CmsUtil.getMainNavList(siteId).get(0).getId();
			return "redirect:"+Global.getFrontPath()+"/list-"+firstCategoryId+Global.getUrlSuffix();
		}else{
			model.addAttribute("site", site);
			return "module/cms/front/themes/"+site.getTheme()+"/frontListCategory";
		}
	}
	...
	public String list(@PathVariable String categoryId, @RequestParam(required=false, defaultValue="1") Integer pageNo,
		...
		if("2".equals(category.getShowModes()) && "article".equals(category.getModule())){
            ...
            CmsUtil.addViewConfigAttribute(model, category);
            CmsUtil.addViewConfigAttribute(model, article.getViewConfig());
			return "module/cms/front/themes/"+site.getTheme()+"/"+getTpl(article);
		}else{
			...
			if("1".equals(category.getShowModes())||categoryList.size()==0){
				...
				return "module/cms/front/themes/"+siteService.get(category.getSite().getId()).getTheme()+view;
				//return "module/cms/front/themes/"+category.getSite().getTheme()+view;
			}
			// 有子栏目：显示子栏目列表
			else{
				model.addAttribute("category", category);
	            ...
	            CmsUtil.addViewConfigAttribute(model, category);
				return "module/cms/front/themes/"+site.getTheme()+view;
			}
		}
	}
	...
	public String listCustom(@PathVariable String categoryId, @PathVariable String customView, @RequestParam(required=false, defaultValue="1") Integer pageNo,
		...
		model.addAttribute("categoryList", categoryList);
        CmsUtil.addViewConfigAttribute(model, category);
		return "module/cms/front/themes/"+site.getTheme()+"/frontListCategory"+customView;
	}
	...
	public String view(@PathVariable String categoryId, @PathVariable String contentId, Model model) {
		...
		if ("article".equals(category.getModule())){
            ...
            CmsUtil.addViewConfigAttribute(model, article.getCategory());
            CmsUtil.addViewConfigAttribute(model, article.getViewConfig());
            Site site = siteService.get(category.getSite().getId());
            model.addAttribute("site", site);
//			return "module/cms/front/themes/"+category.getSite().getTheme()+"/"+getTpl(article);
            return "module/cms/front/themes/"+site.getTheme()+"/"+getTpl(article);
		}
		return "error/404";
	}
	...
	public String comment(String theme, Comment comment, HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		return "module/cms/front/themes/"+theme+"/frontComment";
	}
	...
	public String map(@PathVariable String siteId, Model model) {
		Site site = CmsUtil.getSite(siteId!=null?siteId:Site.defaultSiteId());
		model.addAttribute("site", site);
		return "module/cms/front/themes/"+site.getTheme()+"/frontMap";
	}
